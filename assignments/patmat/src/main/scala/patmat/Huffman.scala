...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] =
    if (chars.isEmpty) throw new NullPointerException
    else
  ...
  def insert(x: Leaf, xs: List[Leaf]): List[Leaf] =

    xs match {
      case List() => List(Leaf(x.char, x.weight))
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case left :: right :: tr => (makeCodeTree(left, right) :: tr)
      .sortWith((tr1, tr2) => weight(tr1) < weight(tr2))
    case _ => trees
  }
  ...
  /**
    ...
    */
  def until(singleTrees: List[CodeTree]=>Boolean, manyTrees: List[CodeTree]=>List[CodeTree])
           (trees: List[CodeTree]): List[CodeTree] = {
    if (singleTrees(trees)) trees
    else until(singleTrees, manyTrees)( manyTrees(trees) )
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def traverse(leftOver: CodeTree, bits: List[Bit]): List[Char] = leftOver match {
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
    type charLists = (Char, List[Bit])
    def prepend(b: Bit)(code: charLists): charLists = (code._1, b :: code._2)
